...
//
// Admin notification bell + popover. Batch N2-D.
//
...
// admin authentication, the 2FA check and the recipient-scope predicate —
// nothing here sends recipient_scope, recipient_id, user_id, user_type or
// client_id, because the server derives all of that from the session.
//
// Deliberately not a poller: the unread count is fetched once on mount, and
// refreshed only when the panel is opened or after a mark-read action.
...
export default function NotificationBell() {
  ...
  // Unread count on mount. Any failure is swallowed into a zero badge — the
  // admin shell must never break because notifications are unavailable.
  useEffect(() => {
    let cancelled = false
  ...
  async function markOneRead(notification: AdminNotification) {
    // Optimistic: flip locally first so the badge responds immediately.
    const alreadyRead = !!notification.read_at
    if (!alreadyRead) {
            ...
            <div className="flex items-center justify-between gap-2 border-b border-[var(--border)] p-3">
              ...
              {notifications.some((n) => !n.read_at) && (
                <button
                  type="button"
                  onClick={markAllRead}
                  className={`rounded-lg px-2 py-1 text-xs font-semibold text-[var(--accent)] hover:bg-[var(--bg-section)] transition ${FOCUS_RING}`}
                >
                  Mark all as read
                </button>
              )}
                    ...
                    >
                      <div className="flex items-start gap-2">
                        {isUnread && (
                          <span
                            aria-hidden="true"
                            className="mt-1.5 h-2 w-2 flex-shrink-0 rounded-full bg-[var(--accent)]"
                          />
                        )}
